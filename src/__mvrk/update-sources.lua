...
-- eli src/__mvrk/update-sources.lua https://gitlab.com/mavryk-network/mavryk-protocol/-/packages/45650499 PtBoreas
...
for platform, sources in pairs(currentSources) do
	...
	for sourceId, _ in pairs(sources) do
		...
		if sourceId:match("baker") or sourceId:match("accuser") then
			assetIds[sourceId] = arch .. "-mavkit-" .. sourceId .. "-" .. protocol
			if protocolNext then
				assetIds[sourceId .. "-next"] = arch .. "-mavkit-" .. sourceId .. "-" .. protocolNext
			end
		end
